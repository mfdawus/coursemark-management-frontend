<template>
  ...
  <div class="bg-white p-4 rounded-2xl shadow">
    <h3 class="text-xl font-semibold mb-2">Avg GPA</h3>
    <p class="text-3xl font-bold">{{ summary.avg_gpa }}</p>
  </div>
</div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-6">
      ...
      <div class="bg-white p-4 rounded-2xl shadow">
        <h4 class="font-semibold mb-2">Avg GPA by Course</h4>
        <BarChart :chartData="gpaChartData" />
      </div>
    </div>
  ...
  <table class="min-w-full divide-y divide-gray-300 rounded-2xl overflow-hidden shadow">
    <thead class="bg-gray-100">
      <tr>
        ...
        <th class="px-4 py-2 text-left font-semibold text-gray-600">GPA</th>
      </tr>
    </thead>
    <tbody class="divide-y divide-gray-200">
      <tr v-for="student in topStudents" :key="student.matric_number"
        ...
        <td class="px-4 py-2">{{ student.gpa?.toFixed(2) ?? '-' }}</td>
      </tr>
    </tbody>
  </table>
</div>
