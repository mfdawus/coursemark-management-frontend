    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Notes for {{ student?.student_name || 'Student' }}</h5>
        ...
        <div class="modal-body">
          ...
          <textarea v-model="newNote" class="form-control mb-2" rows="3" placeholder="Add a new note..."></textarea>
          <input type="date" v-model="meetingDate" class="form-control mb-2" />
          <button class="btn btn-primary w-100" @click="saveNote" :disabled="!newNote.trim() || loading || !selectedCourseId">
...
async function loadCourses() {
  ...
  const res = await fetch(`/api/advisor/student-courses?student_id=${encodeURIComponent(props.student.student_id)}`);
  if (res.ok) {
...
async function loadNotes() {
  ...
  try {
    ...
    const res = await fetch(`/api/advisor/notes?student_id=${encodeURIComponent(props.student.student_id)}`);
    if (res.ok) {
...
async function saveNote() {
  ...
  try {
    ...
    const res = await fetch(`/api/advisor/notes-by-matric/${encodeURIComponent(matric_number)}/${encodeURIComponent(course_id)}`, {
      method: 'POST',
